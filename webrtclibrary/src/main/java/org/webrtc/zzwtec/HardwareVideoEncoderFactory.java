...
import static org.webrtc.zzwtec.MediaCodecUtils.EXYNOS_PREFIX;
import static org.webrtc.zzwtec.MediaCodecUtils.INTEL_PREFIX;
import static org.webrtc.zzwtec.MediaCodecUtils.QCOM_PREFIX;
...
public class HardwareVideoEncoderFactory implements VideoEncoderFactory {
  ...
  public VideoEncoder createEncoder(VideoCodecInfo input) {
    ...
    org.webrtc.zzwtec.VideoCodecType type = org.webrtc.zzwtec.VideoCodecType.valueOf(input.name);
    ...
    Integer surfaceColorFormat = org.webrtc.zzwtec.MediaCodecUtils.selectColorFormat(
        org.webrtc.zzwtec.MediaCodecUtils.TEXTURE_COLOR_FORMATS, info.getCapabilitiesForType(mime));
    Integer yuvColorFormat = org.webrtc.zzwtec.MediaCodecUtils.selectColorFormat(
        org.webrtc.zzwtec.MediaCodecUtils.ENCODER_COLOR_FORMATS, info.getCapabilitiesForType(mime));

    if (type == org.webrtc.zzwtec.VideoCodecType.H264) {
      boolean isHighProfile = H264Utils.isSameH264Profile(
          input.params, org.webrtc.zzwtec.MediaCodecUtils.getCodecProperties(type, /* highProfile= */ true));
      boolean isBaselineProfile = H264Utils.isSameH264Profile(
          input.params, org.webrtc.zzwtec.MediaCodecUtils.getCodecProperties(type, /* highProfile= */ false));
    ...
    return new org.webrtc.zzwtec.HardwareVideoEncoder(new org.webrtc.zzwtec.MediaCodecWrapperFactoryImpl(), codecName, type,
  ...
  public VideoCodecInfo[] getSupportedCodecs() {
    ...
    for (org.webrtc.zzwtec.VideoCodecType type :
        new org.webrtc.zzwtec.VideoCodecType[] {org.webrtc.zzwtec.VideoCodecType.VP8, org.webrtc.zzwtec.VideoCodecType.VP9, org.webrtc.zzwtec.VideoCodecType.H264}) {
      MediaCodecInfo codec = findCodecForType(type);
      if (codec != null) {
        String name = type.name();
        // TODO(sakal): Always add H264 HP once WebRTC correctly removes codecs that are not
        // supported by the decoder.
        if (type == org.webrtc.zzwtec.VideoCodecType.H264 && isH264HighProfileSupported(codec)) {
          supportedCodecInfos.add(new VideoCodecInfo(
              name, org.webrtc.zzwtec.MediaCodecUtils.getCodecProperties(type, /* highProfile= */ true)));
        }

        supportedCodecInfos.add(new VideoCodecInfo(
            name, org.webrtc.zzwtec.MediaCodecUtils.getCodecProperties(type, /* highProfile= */ false)));
      }
    }
  ...
  private @Nullable MediaCodecInfo findCodecForType(org.webrtc.zzwtec.VideoCodecType type) {
  ...
  private boolean isSupportedCodec(MediaCodecInfo info, org.webrtc.zzwtec.VideoCodecType type) {
    if (!org.webrtc.zzwtec.MediaCodecUtils.codecSupportsType(info, type)) {
      return false;
    }
    // Check for a supported color format.
    if (org.webrtc.zzwtec.MediaCodecUtils.selectColorFormat(
            org.webrtc.zzwtec.MediaCodecUtils.ENCODER_COLOR_FORMATS, info.getCapabilitiesForType(type.mimeType()))
        == null) {
      return false;
    }
  ...
  // current SDK.
  private boolean isHardwareSupportedInCurrentSdk(MediaCodecInfo info, org.webrtc.zzwtec.VideoCodecType type) {
    switch (type) {
      case VP8:
  ...
  private int getKeyFrameIntervalSec(org.webrtc.zzwtec.VideoCodecType type) {
    switch (type) {
  ...
  private int getForcedKeyFrameIntervalMs(org.webrtc.zzwtec.VideoCodecType type, String codecName) {
    if (type == org.webrtc.zzwtec.VideoCodecType.VP8 && codecName.startsWith(QCOM_PREFIX)) {
  ...
  private BitrateAdjuster createBitrateAdjuster(org.webrtc.zzwtec.VideoCodecType type, String codecName) {
    if (codecName.startsWith(EXYNOS_PREFIX)) {
      if (type == org.webrtc.zzwtec.VideoCodecType.VP8) {
        // Exynos VP8 encoders need dynamic bitrate adjustment.
        return new org.webrtc.zzwtec.DynamicBitrateAdjuster();
      } else {
        // Exynos VP9 and H264 encoders need framerate-based bitrate adjustment.
        return new org.webrtc.zzwtec.FramerateBitrateAdjuster();
      }
    }
    // Other codecs don't need bitrate adjustment.
    return new org.webrtc.zzwtec.BaseBitrateAdjuster();
  }
